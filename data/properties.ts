export const properties1 = [
  {
    id: 1,
    slug: "almalqa-center",
    title: { en: "Almalqa Center", ar: "مركز الملقا" },
    status: { en: "For Rent", ar: "للإيجار" },
    category: "Commercial",
    area: "18041.610 m²",
    location: { en: "Riyadh, Saudi Arabia", ar: "الرياض، المملكة العربية السعودية" },
    coordinates: { lat: 24.774265, lng: 46.738586 },
    gallery: ["/image3.jpeg", "/image3.jpeg", "/image3.jpeg"],
    description: {
      en: "Modern commercial center offering premium office and showroom spaces in a strategic location.",
      ar: "مركز تجاري حديث يوفر مساحات مكتبية ومعارض بمواصفات عالية في موقع استراتيجي.",
    },
    overview: {
      en: ["Prime location", "High-quality construction", "Flexible layouts", "Investment-ready"],
      ar: ["موقع استراتيجي", "جودة بناء عالية", "تصاميم مرنة", "فرصة استثمارية"],
    },
    specifications: {
      propertyType: { en: "Commercial Center", ar: "مركز تجاري" },
      totalUnits: "40 Offices | 45 Showrooms",
      parkingSpaces: 120,
      completionYear: 2023,
    },
    amenities: {
      en: ["Security", "Parking", "Elevator", "CCTV"],
      ar: ["أمن", "مواقف سيارات", "مصاعد", "كاميرات مراقبة"],
    },
    investmentInfo: {
      en: "Strategically located for high rental yield.",
      ar: "موقع استراتيجي يحقق عوائد إيجارية مرتفعة.",
    },
    developer: "NAYAN Real Estate",
    image: "/image3.jpeg",
  },

  {
    id: 2,
    slug: "riyadh-garden-residences",
    title: { en: "Riyadh Garden Residences", ar: "حدائق الرياض السكنية" },
    status: { en: "Completed", ar: "مكتمل" },
    category: "Apartments",
    area: "9500 m²",
    location: { en: "Al Narjis, Riyadh", ar: "حي النرجس، الرياض" },
    coordinates: { lat: 24.800000, lng: 46.700000 },
    gallery: ["/image3.jpeg", "/image3.jpeg"],
    description: {
      en: "Luxury apartments designed for modern urban living.",
      ar: "شقق فاخرة مصممة للحياة الحضرية العصرية.",
    },
    overview: {
      en: ["Modern design", "Family-friendly", "Premium finishes"],
      ar: ["تصميم عصري", "مناسب للعائلات", "تشطيبات فاخرة"],
    },
    specifications: {
      propertyType: { en: "Residential Apartments", ar: "شقق سكنية" },
      totalUnits: "120 Units",
      parkingSpaces: 200,
      completionYear: 2022,
    },
    amenities: {
      en: ["Gym", "Pool", "Security", "Parking"],
      ar: ["نادي رياضي", "مسبح", "أمن", "مواقف سيارات"],
    },
    investmentInfo: {
      en: "High demand residential area.",
      ar: "منطقة سكنية ذات طلب مرتفع.",
    },
    developer: "NAYAN Real Estate",
    image: "/image3.jpeg",
  },

  {
    id: 3,
    slug: "urban-duplex-living",
    title: { en: "Urban Duplex Living", ar: "الدوبلكس الحضري" },
    status: { en: "For Sale", ar: "للبيع" },
    category: "Duplex",
    area: "1200 m²",
    location: { en: "Al Malqa, Riyadh", ar: "حي الملقا، الرياض" },
    coordinates: { lat: 24.820000, lng: 46.680000 },
    gallery: ["/image3.jpeg"],
    description: {
      en: "Contemporary duplex homes with spacious layouts.",
      ar: "منازل دوبلكس عصرية بتصاميم واسعة.",
    },
    overview: {
      en: ["Spacious", "Modern architecture"],
      ar: ["مساحات واسعة", "تصميم معماري عصري"],
    },
    specifications: {
      propertyType: { en: "Duplex Villa", ar: "فيلا دوبلكس" },
      totalUnits: "20 Units",
      parkingSpaces: 40,
      completionYear: 2024,
    },
    amenities: {
      en: ["Private Garden", "Parking", "Security"],
      ar: ["حديقة خاصة", "مواقف", "أمن"],
    },
    investmentInfo: {
      en: "Perfect for families.",
      ar: "مثالي للعائلات.",
    },
    developer: "NAYAN Real Estate",
    image: "/image3.jpeg",
  },

  {
    id: 4,
    slug: "jeddah-seaside-resort",
    title: { en: "Jeddah Seaside Resort", ar: "منتجع جدة الساحلي" },
    status: { en: "Under Construction", ar: "قيد الإنشاء" },
    category: "Resort",
    area: "15000 m²",
    location: { en: "Jeddah Waterfront", ar: "واجهة جدة البحرية" },
    coordinates: { lat: 21.543333, lng: 39.172779 },
    gallery: ["/image3.jpeg"],
    description: {
      en: "Luxury seaside resort with world-class facilities.",
      ar: "منتجع ساحلي فاخر بمرافق عالمية.",
    },
    overview: {
      en: ["Seaside view", "Premium facilities"],
      ar: ["إطلالة بحرية", "مرافق فاخرة"],
    },
    specifications: {
      propertyType: { en: "Resort Development", ar: "منتجع سياحي" },
      totalUnits: "60 Villas",
      parkingSpaces: 150,
      completionYear: 2026,
    },
    amenities: {
      en: ["Beach Access", "Pool", "Spa"],
      ar: ["وصول مباشر للشاطئ", "مسبح", "سبا"],
    },
    investmentInfo: {
      en: "Tourism investment opportunity.",
      ar: "فرصة استثمارية سياحية.",
    },
    developer: "NAYAN Real Estate",
    image: "/image3.jpeg",
  },

  {
    id: 5,
    slug: "north-riyadh-warehouse",
    title: { en: "North Riyadh Warehouse Complex", ar: "مجمع مستودعات شمال الرياض" },
    status: { en: "For Lease", ar: "للإيجار طويل الأجل" },
    category: "Warehouse",
    area: "22000 m²",
    location: { en: "Industrial City, Riyadh", ar: "المدينة الصناعية، الرياض" },
    coordinates: { lat: 24.650000, lng: 46.750000 },
    gallery: ["/image3.jpeg"],
    description: {
      en: "Strategic warehouse complex for logistics.",
      ar: "مجمع مستودعات استراتيجي للخدمات اللوجستية.",
    },
    overview: {
      en: ["Logistics ready", "Large capacity"],
      ar: ["جاهز لوجستياً", "سعة كبيرة"],
    },
    specifications: {
      propertyType: { en: "Industrial Warehouse", ar: "مستودعات صناعية" },
      totalUnits: "15 Warehouses",
      parkingSpaces: 80,
      completionYear: 2021,
    },
    amenities: {
      en: ["24/7 Security", "Loading docks"],
      ar: ["أمن 24/7", "مناطق تحميل"],
    },
    investmentInfo: {
      en: "Strong industrial demand.",
      ar: "طلب صناعي مرتفع.",
    },
    developer: "NAYAN Real Estate",
    image: "/image3.jpeg",
  },

  {
    id: 6,
    slug: "luxury-villas-al-yasmin",
    title: { en: "Luxury Villas Al Yasmin", ar: "فلل الياسمين الفاخرة" },
    status: { en: "Completed", ar: "مكتمل" },
    category: "Luxury Villas",
    area: "8000 m²",
    location: { en: "Al Yasmin, Riyadh", ar: "حي الياسمين، الرياض" },
    coordinates: { lat: 24.830000, lng: 46.670000 },
    gallery: ["/image3.jpeg"],
    description: {
      en: "Exclusive luxury villas with premium amenities.",
      ar: "فلل فاخرة بمرافق راقية.",
    },
    overview: {
      en: ["Private pools", "Premium finishes"],
      ar: ["مسبح خاص", "تشطيبات فاخرة"],
    },
    specifications: {
      propertyType: { en: "Luxury Villa", ar: "فيلا فاخرة" },
      totalUnits: "10 Villas",
      parkingSpaces: 30,
      completionYear: 2022,
    },
    amenities: {
      en: ["Pool", "Smart Home", "Security"],
      ar: ["مسبح", "منزل ذكي", "أمن"],
    },
    investmentInfo: {
      en: "High-end residential investment.",
      ar: "استثمار سكني راقٍ.",
    },
    developer: "NAYAN Real Estate",
    image: "/image3.jpeg",
  },

  {
    id: 7,
    slug: "business-hub-center",
    title: { en: "Business Hub Center", ar: "مركز محور الأعمال" },
    status: { en: "For Rent", ar: "للإيجار" },
    category: "Offices",
    area: "11000 m²",
    location: { en: "King Fahd Road, Riyadh", ar: "طريق الملك فهد، الرياض" },
    coordinates: { lat: 24.700000, lng: 46.680000 },
    gallery: ["/image3.jpeg"],
    description: {
      en: "Corporate office spaces in prime district.",
      ar: "مساحات مكتبية للشركات في موقع مميز.",
    },
    overview: {
      en: ["Corporate ready", "Central location"],
      ar: ["جاهز للشركات", "موقع مركزي"],
    },
    specifications: {
      propertyType: { en: "Office Tower", ar: "برج مكاتب" },
      totalUnits: "80 Offices",
      parkingSpaces: 180,
      completionYear: 2020,
    },
    amenities: {
      en: ["Security", "Elevators", "Conference rooms"],
      ar: ["أمن", "مصاعد", "قاعات اجتماعات"],
    },
    investmentInfo: {
      en: "Stable rental income opportunity.",
      ar: "فرصة دخل إيجاري مستقر.",
    },
    developer: "NAYAN Real Estate",
    image: "/image3.jpeg",
  },

  {
    id: 8,
    slug: "executive-apartment-tower",
    title: { en: "Executive Apartment Tower", ar: "برج الشقق التنفيذية" },
    status: { en: "Under Construction", ar: "قيد الإنشاء" },
    category: "Apartments",
    area: "13500 m²",
    location: { en: "Olaya District, Riyadh", ar: "حي العليا، الرياض" },
    coordinates: { lat: 24.690000, lng: 46.670000 },
    gallery: ["/image3.jpeg"],
    description: {
      en: "High-rise tower offering executive-level living.",
      ar: "برج شاهق يوفر مستوى معيشة تنفيذي.",
    },
    overview: {
      en: ["Luxury lifestyle", "Central district"],
      ar: ["أسلوب حياة فاخر", "موقع مركزي"],
    },
    specifications: {
      propertyType: { en: "Residential Tower", ar: "برج سكني" },
      totalUnits: "150 Units",
      parkingSpaces: 250,
      completionYear: 2026,
    },
    amenities: {
      en: ["Gym", "Pool", "Security"],
      ar: ["نادي رياضي", "مسبح", "أمن"],
    },
    investmentInfo: {
      en: "Ideal for executive tenants.",
      ar: "مثالي للمستأجرين التنفيذيين.",
    },
    developer: "NAYAN Real Estate",
    image: "/image3.jpeg",
  },
];


export const categories = ["All", "Apartments", "Villas", "Duplex", "Offices", "Land"];